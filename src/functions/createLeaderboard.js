...
module.exports = async (client, interaction, title, lb, ephemeral = false) => {
  ...
  let button2 = new ButtonBuilder()
    ...
    .setStyle(2)
    .setDisabled(true);
  let Cbtn = new ButtonBuilder()
    .setLabel(`${page}/${Math.ceil(lb.length / 10)}`)
    .setDisabled(true)
    .setStyle(2)
  ...
  let row = new ActionRowBuilder()
    .addComponents(button1, button2, Cbtn, button3, button4);
  ...
  collector.on('collect', async (btn) => {
    if (btn.user.id == interaction.user.id) {
      ...
      let row2 = new ActionRowBuilder()
        .addComponents(btn1, btn2, Cbtn2, btn3, btn4);
      interaction.editReply({ fetchReply: true, embeds: [generateEmbed(currentIndex, currentIndex, lb, title, page)], components: [row2] });
      btn.deferUpdate();
    }
  })
