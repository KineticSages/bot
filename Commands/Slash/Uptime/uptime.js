...
module.exports = {
  name: "uptime",
  description: "moniter your commands to keep your website online.",
  usage: "",
  category: "uptime",
  userPerms: [""],
  botPerms: [""],
  ...
  run: async (client, interaction) => {
    try {
      ...
      if (sub === "total") {
              ...
              embeds: [
                client.Embed()
                  .setAuthor({
                    ...
                    url: "https://discord.gg/dfShKEzmBE",
        ...
        const link_to_moniter = new TextInputBuilder()
          ...
          .setStyle(1)
          .setMinLength(0)
          .setPlaceholder('https://uptime.js.cool')
          .setRequired(true);
    ...
    all.forEach(async (data) => {
      ...
      if (data.get("error")) {
        embed.addField(
          `**${count}**. \`${doc.URL}\``,
          `<:ram:1020295270786924624> Last Pinged: ${
    ...
    await interaction.author.send(embed).catch((err) => {
      errors = true;
      if (err.interaction === "Cannot send messages to this user")
        return interaction.reply(
