...
module.exports = {
  name: 'test',
  description: "Shows you Bot Commands",
  category: "info",
  cooldown: 10,
  guildOnly: false,
  ownerOnly: true,
  ...
  run: async (client, interaction) => {
    try {
      const prefix_cmd_count = client.commands.map(a => a).length;
      const slash_cmd_count = client.slashCommands.map(a => a).length;
      const prefix_ownerOnly = client.commands.filter((cmd) => cmd.category === "ownerOnly").size || 0;
      const slash_ownerOnly = client.slashCommands.filter((cmd) => cmd.category === "ownerOnly").size || 0;
      // first main page 🏆
      let em1 = client.Embed(false)
        .setAuthor({ name: `${client.user.username}\'s Help Menu`, iconURL: client.user.displayAvatarURL({ format: "png" }), url: "https://dsc.gg/spicydevs" })
        .setDescription(`• Prefix of this server is: \`${client.id}\`\n• Total Commands: ${prefix_cmd_count + slash_cmd_count || 0} | Unable to you (here): ${prefix_ownerOnly + slash_ownerOnly}\n• [Invite ${client.user.username}](${client.user.tag}) | [Support Server](https://spicydevs.me)`)
        .addFields([
          { name: `Categories`, value: `>>> 💀 Information\n💀 Music\n💀 Ranking\n💀 Economy`, inline: true },
          { name: `Extra Categories`, value: `>>> <:school1:954251695788011560> Utility\n<:nsfw:992412474806640701> NSFW\n<:fun:992383624462729246> Fun\n🎟️ Ticket`, inline: true },
          { name: `Stats`, value: `>>> Ping: ${Math.floor(client.ws.ping)}ms\nGuilds: ${client.guilds.cache.size}\nMembers: ${client.users.cache.size}`, inline: true }
        ])
      // info commands 🎃
